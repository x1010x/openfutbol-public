...
export const TournamentSetupView = ({ onConfirm, onBack }: Props) => {
  ...
  const [stages, setStages] = useState<StageDraft[]>([{ kind: 'ko', legs: 1 }]);
  ...
  const validation = useMemo(() => validateStageChain(n, stages), [n, stages]);
  ...
  const canConfirm = validation.ok && n >= 2 && (spectate || !!userClubId);

  const applyPreset = (drafts: StageDraft[]) => setStages(drafts);

  const addStage = (kind: 'liga' | 'ko') => {
    if (kind === 'liga') setStages(prev => [...prev, { kind: 'liga', groupSize: 4, advancePerGroup: 2 }]);
    else setStages(prev => [...prev, { kind: 'ko', legs: 1 }]);
  };
      ...
      <div className="bg-vga-gray border-4 border-vga-blue p-4 flex flex-col gap-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="flex flex-col gap-1">
            <label className="flex items-center gap-2 text-vga-bright-white text-[9px] uppercase">
              <input
                type="checkbox"
                checked={spectate}
                onChange={e => { setSpectate(e.target.checked); if (e.target.checked) setUserClubId(null); }}
              />
              Espectador
            </label>
            <label className="flex items-center gap-2 text-vga-bright-white text-[9px] uppercase">
        ...
        <div className="border-2 border-vga-blue bg-vga-black p-3 flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <div className="text-vga-cyan text-[9px] uppercase tracking-widest">Fases del torneo</div>
            <div className="flex gap-1">
              <button onClick={() => applyPreset(presetCopa(Math.max(2, n)))} className="px-2 py-1 text-[8px] uppercase border border-vga-blue bg-vga-black text-vga-cyan hover:border-vga-yellow">
                Preset Copa
              </button>
              <button onClick={() => applyPreset(presetMundial(Math.max(4, n)))} className="px-2 py-1 text-[8px] uppercase border border-vga-blue bg-vga-black text-vga-cyan hover:border-vga-yellow">
                Preset Mundial
              </button>
            </div>
          </div>
            ...
            return (
              <div key={idx} className="border border-vga-blue p-2 flex flex-col gap-1">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-vga-yellow text-[9px] uppercase font-bold">Fase {idx + 1}</span>
                  <div className="flex gap-1">
                    <button
                      onClick={() => updateStage(idx, s.kind === 'liga' ? { kind: 'ko', legs: 1 } : { kind: 'liga', groupSize: 4, advancePerGroup: 2 })}
                      className="px-2 py-0.5 text-[8px] uppercase border border-vga-blue bg-vga-black text-vga-cyan hover:border-vga-yellow"
                    >
                      {s.kind === 'liga' ? 'Cambiar a KO' : 'Cambiar a Liga'}
                    </button>
                    <button
                ...
                <div className="text-[8px] text-vga-gray uppercase">
                  {io ? `${io.input} → ${io.output}` : '—'}
                  {s.kind === 'liga' && io && ` · ${io.input / (s.groupSize ?? 4)} grupo${(io.input / (s.groupSize ?? 4)) === 1 ? '' : 's'}`}
                </div>
              </div>
            );
          })}
          ...
          <div className="flex gap-2 mt-1">
            <button onClick={() => addStage('liga')} className="px-3 py-1 text-[9px] uppercase border border-vga-blue bg-vga-black text-vga-cyan hover:border-vga-yellow">
              + Liga
            </button>
            <button onClick={() => addStage('ko')} className="px-3 py-1 text-[9px] uppercase border border-vga-blue bg-vga-black text-vga-cyan hover:border-vga-yellow">
              + Eliminatoria
            </button>
          </div>

          {!validation.ok && (
            <div className="text-vga-light-red text-[8px] uppercase mt-1">{validation.reason}</div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-2 border-t-2 border-vga-blue pt-2">
          <div className="text-[9px] text-vga-bright-white uppercase">
            {n} equipo{n === 1 ? '' : 's'} · {stages.length} fase{stages.length === 1 ? '' : 's'}
            {spectate ? ' · modo espectador' : userClubId ? ' · tu equipo elegido' : ' · elige tu equipo'}
          </div>
          <button
