...
export const SquadView = ({
  ...
  return (
    <div className="w-full max-w-5xl flex flex-col gap-4">
      ...
      {showOffers && (
        <div className="border-2 border-vga-yellow p-2 bg-vga-blue">
          <h3 className="text-vga-yellow text-[10px] font-bold mb-2 uppercase">
            Ofertas recibidas <span className="text-vga-gray font-normal text-[8px]">({affordableOffers.length})</span>
          </h3>
          <div className="flex flex-col gap-1">
            {/* Group by player */}
            {Array.from(new Map(affordableOffers.map(o => [o.playerId, o])).keys()).map(playerId => {
              const playerOffers = affordableOffers.filter(o => o.playerId === playerId);
              const player = userPlayerById(playerId);
              if (!player) return null;
              const isOpen = expandedOfferPlayers.has(playerId);
              const bestOffer = playerOffers.reduce((best, o) => o.amount > best.amount ? o : best, playerOffers[0]);
              const price = computePrice(player, seasonYear);
              const bestRatio = bestOffer.amount / price;
              const ratioClass = bestRatio >= 1.5 ? 'text-vga-light-green' : bestRatio >= 1 ? 'text-vga-yellow' : 'text-vga-light-red';
              return (
                <div key={playerId} className="border border-vga-yellow">
                  {/* Collapsed header */}
                  <button
                    onClick={() => toggleOfferPlayer(playerId)}
                    className="w-full flex items-center justify-between px-2 py-1.5 bg-vga-black hover:bg-vga-blue text-left gap-2"
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className={`text-[8px] font-bold ${ratioClass}`}>{formatEuros(bestOffer.amount)}</span>
                      <span className="text-vga-gray text-[8px]">{isOpen ? '▲' : '▼'}</span>
                    </div>
                  </button>

                  {/* Expanded offers */}
                  {isOpen && playerOffers.map(offer => {
              ...
              const ratioClass = ratio >= 1.5 ? 'text-vga-light-green' : ratio >= 1 ? 'text-vga-yellow' : 'text-vga-light-red';
              ...
              return (
                <div key={offer.id} className="bg-vga-black border border-vga-gray text-[8px] flex flex-col">
                  <div className="p-2 flex items-center justify-between gap-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[7px] text-vga-cyan mt-0.5">
                        ...
                        <span className="ml-2">VALOR <span className="text-vga-light-green">{formatEuros(price)}</span></span>
                      ...
                      <div className="text-[9px] mt-0.5">
                        {isSwap ? 'TOTAL' : 'OFERTA'}{' '}
                        <span className={`font-bold ${ratioClass}`}>{formatEuros(totalValue)}</span>
