...
const barColor = (pct: number) => {
  if (pct >= 0.8) return 'bg-vga-light-green';
  if (pct >= 0.6) return 'bg-vga-yellow';
  if (pct >= 0.4) return 'bg-vga-brown';
  return 'bg-vga-light-red';
};

export const StatBar = ({ label, value, max = 100, segments = 10, size = 'md' }: Props) => {
  const pct = value / max;
